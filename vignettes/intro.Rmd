...
higly-variable genes, in order to remove transcriptional noise and focus on the
...
```{r zinb}
library(zinbwave)
zinb <- zinbFit(assay(fluidigm), K=2, epsilon=1000)
```
...
We first create a design matrix, which includes an intercept and an indicator
variable for the coverage.

```{r design}
coverage <- colData(fluidigm)$Coverage_Type
modX <- model.matrix(~coverage)
head(modX)
```

We can then pass this matrix to `zinbFit`.

```{r zinb_coverage}
zinb_cov <- zinbFit(assay(fluidigm), K=2, X=modX, epsilon=1000)
```
...
Anagously, we can include gene-level covariates, as columns of $V$. Here, we 
...
bm <- getBM(attributes=c('hgnc_symbol', 'start_position',
                         'end_position', 'percentage_gc_content'),
            filters = 'hgnc_symbol',
...
gcc <- tapply(bm$percentage_gc_content, bm$hgnc_symbol, mean)
gcc <- gcc[rownames(fluidigm)]

modV <- model.matrix(~gcc + log(len))
head(modV)
```

```{r zinb_gcc}
zinb_gcc <- zinbFit(assay(fluidigm), K=2, V=modV, epsilon=1000)
```
